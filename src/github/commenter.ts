...
export class PRCommenter {
  ...
  formatAnalysisComment(
    ...
    let comment = `## 🔍 Supabase SQL Schema Analysis\n\n`;

    // AI Model metadata
    if (aiModel && modelVersion) {
      comment += `**🤖 AI Model:** ${aiModel}\n`;
      comment += `**📌 Version:** \`${modelVersion}\`\n\n`;
    }
    ...
    comment += `| 🚨 Critical Issues | ${totalCritical} |\n`;
    comment += `| ⚠️ Warnings | ${totalWarnings} |\n`;
    comment += `| ℹ️ Suggestions | ${totalSuggestions} |\n`;
    comment += `| ⏱️ Est. Fix Time | ~${estimatedTime} min |\n\n`;

    // Status Badge
    if (totalCritical > 0) {
      comment += `> **🚨 ACTION REQUIRED** - ${totalCritical} critical issue(s) must be resolved before merging\n\n`;
    } else if (totalWarnings > 0) {
      comment += `> **✅ READY** - No critical issues, ${totalWarnings} warning(s) to consider\n\n`;
    } else {
      comment += `> **🎉 EXCELLENT** - All files follow Supabase best practices!\n\n`;
    }

    if (runUrl) {
      comment += `[📋 View detailed report in GitHub Actions](${runUrl})\n\n`;
    }
    ...
    const modelText = aiModel && modelVersion ? `${aiModel} (${modelVersion})` : 'AI';
    comment += `*Analysis powered by ${modelText} | Generated by [Supabase SQL Agent](https://github.com/${this.owner}/${this.repo})*`;

    return comment;
  }

  private formatFileAnalysis(filename: string, result: AnalysisResult): string {
    let section = `### 📄 \`${filename}\` - Score: **${result.score}/10**\n\n`;

    section += `**Summary:** ${result.summary}\n\n`;

    // Critical Issues
    if (result.critical.length > 0) {
      section += `#### 🚨 Critical Issues (${result.critical.length})\n\n`;
    ...
    if (result.suggestions.length > 0) {
      section += `#### ℹ️ Suggestions (${result.suggestions.length})\n\n`;
  ...
  async postComment(
    ...
    try {
      ...
      const modelText = aiModel ? `[${aiModel}] ` : '';
      console.log(`✅ ${modelText}Comment posted to PR #${prNumber}`);
    } catch (error) {
